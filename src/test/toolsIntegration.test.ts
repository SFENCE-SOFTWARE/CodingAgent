...
suite('Tools Integration Tests', () => {
    ...
    test('should register all expected tools', () => {
        ...
        assert.ok(toolsInfo.length >= expectedToolNames.length, 
            `Should have at least ${expectedToolNames.length} tools, got ${toolsInfo.length}`);
    });
    ...
    test('should provide valid tool definitions for all tools', () => {
        ...
        for (const info of toolsInfo) {
            ...
            assert.ok(['file', 'system', 'web', 'search', 'other'].includes(info.category), 
                `Tool category ${info.category} should be valid`);
            ...
            assert.ok(definition.function.description.length > 0, 
                'Tool function description should not be empty');
            ...
            assert.ok(Array.isArray(definition.function.parameters.required), 
                'Required parameters should be an array');
            assert.strictEqual(typeof definition.function.parameters.properties, 'object');
            assert.ok(definition.function.parameters.properties !== null, 
                'Parameters properties should not be null');
        }
    });
    ...
    test('should provide tools organized by category', () => {
        ...
        assert.ok(categories.includes('system') || categories.includes('search'), 
            'Should have system or search category tools');
        
        // Each category should have tools
        for (const category of categories) {
            assert.ok(toolsByCategory[category].length > 0, 
        ...
        for (const info of toolsInfo) {
            ...
            try {
                ...
                assert.ok(error instanceof Error, 
                    `Tool ${info.name} should throw proper Error objects`);
            }
        }
    });
