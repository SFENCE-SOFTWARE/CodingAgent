...
export class PlanEvaluateTool implements BaseTool {
  ...
  getToolDefinition(): ToolDefinition {
    return {
      ...
      function: {
        name: 'plan_evaluate',
        description: 'Evaluates plan completion status and generates corrective prompts if needed. Checks in priority order: plan needs rework -> plan not reviewed -> points need rework -> points not reviewed -> points not tested -> points not implemented -> plan accepted. Returns only the first relevant point for each step to focus workflow. Note: Points which are not implemented cannot be marked as not reviewed or not tested.',
        parameters: {
          type: 'object',
          properties: {
            plan_id: {
